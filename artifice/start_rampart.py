...
def start_rampart(path, firstPort = 1100, secondPort = 1200, test = False):
    ...
    os.system(command)
    if test:
        command = f"docker run --name {containerName} -p {secondPort}:{secondPort} -p {firstPort}:{firstPort} -e PORT_ONE={firstPort} -e PORT_TWO={secondPort} --volume {path}:/data/run_data/ rampart_test"
